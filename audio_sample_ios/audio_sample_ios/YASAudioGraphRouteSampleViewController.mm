...
struct route_vc_cpp {
    ...
    void disconnectNodes() {
        ...
        graph->disconnect(this->graph->io().value()->node());
    }

    void connect_nodes() {
        if (auto const format = this->device->output_format()) {
            graph->connect(this->au_mixer->raw_au()->node(), this->graph->io().value()->node(), *format);
        ...
        if (auto const format = this->device->input_format()) {
            graph->connect(this->graph->io().value()->node(), this->route->node(), 0,
