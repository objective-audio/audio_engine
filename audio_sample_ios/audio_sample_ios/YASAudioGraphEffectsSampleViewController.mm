            ...
            this->_pool += effect_au->load_state_chain()
                               .perform([this, format](auto const &state) {
                                   if (state == audio::avf_au::load_state::loaded) {
                                       this->graph->connect(this->effect_au.value()->node(),
                                                            this->graph->io().value()->node(), format);
                                       this->graph->connect(tap->node(), this->effect_au.value()->node(), format);
                                   }
                               })
                               .sync();
        } else {
            this->through_connection = graph->connect(this->tap->node(), this->graph->io().value()->node(), format);
        }
    }
};
}
