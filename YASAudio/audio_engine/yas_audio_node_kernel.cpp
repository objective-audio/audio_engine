...
audio::connection_smap audio_node::kernel::input_connections() const
{
    return yas::lock_values(_impl->input_connections);
}

audio::connection_smap audio_node::kernel::output_connections() const
{
    return yas::lock_values(_impl->output_connections);
}

audio::connection audio_node::kernel::input_connection(const UInt32 bus_idx)
{
...
audio::connection audio_node::kernel::output_connection(const UInt32 bus_idx)
{
...
void audio_node::kernel::_set_input_connections(const audio::connection_wmap &connections)
{
    _impl->input_connections = connections;
}

void audio_node::kernel::_set_output_connections(const audio::connection_wmap &connections)
{
