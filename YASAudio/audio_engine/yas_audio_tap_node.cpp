...
audio::tap_node::tap_node(const std::shared_ptr<impl> &impl) : super_class(impl)
{
}

audio::tap_node::~tap_node() = default;

void audio::tap_node::set_render_function(const render_f &func)
{
    impl_ptr<impl>()->set_render_function(func);
}

audio::connection audio::tap_node::input_connection_on_render(const UInt32 bus_idx) const
{
    return impl_ptr<impl>()->input_connection_on_render(bus_idx);
}

audio::connection audio::tap_node::output_connection_on_render(const UInt32 bus_idx) const
{
...
void audio::tap_node::render_source(audio::pcm_buffer &buffer, const UInt32 bus_idx, const audio::time &when)
{
