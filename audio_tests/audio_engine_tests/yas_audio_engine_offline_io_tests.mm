//
//  yas_audio_engine_offline_io_tests.mm
//
...
@interface yas_audio_engine_offline_io_tests : XCTestCase

@end

@implementation yas_audio_engine_offline_io_tests
...
- (void)test_offline_render_with_audio_engine {
    ...
    auto tap_render_handler = [&self, &tapNodeExpectation, &tap_render_frame, &sample_rate, &frames_per_render,
                               &format](audio::engine::node::render_args args) {
    ...
    manager->connect(sample_delay_au->node(), offline_io->node(), format);
    ...
    XCTAssertTrue(manager->io().value()->raw_io()->is_running());

    [self waitForExpectationsWithTimeout:10.0 handler:nil];

    XCTAssertFalse(manager->io().value()->raw_io()->is_running());
}
