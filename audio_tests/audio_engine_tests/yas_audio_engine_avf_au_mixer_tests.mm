//
//  yas_audio_avf_au_mixer_tests.m
//
...
@interface yas_audio_avf_au_mixer_tests : XCTestCase

@end

@implementation yas_audio_avf_au_mixer_tests
...
- (void)test_parameter_exists {
    auto au_mixer = audio::engine::avf_au_mixer::make_shared();

    auto const &input_parameters = au_mixer->au()->raw_au()->input_parameters();
    auto const &output_parameters = au_mixer->au()->raw_au()->output_parameters();

    XCTAssertGreaterThanOrEqual(input_parameters.size(), 7);
    XCTAssertGreaterThanOrEqual(output_parameters.size(), 2);

    auto const &raw_au = au_mixer->au()->raw_au();
...
- (void)test_bus {
    auto au_mixer = audio::engine::avf_au_mixer::make_shared();
    uint32_t const default_bus_count = au_mixer->au()->raw_au()->input_bus_count();
    ...
    XCTAssertNoThrow(au_mixer->au()->raw_au()->set_input_bus_count(bus_count));

    XCTAssertGreaterThanOrEqual(au_mixer->au()->raw_au()->input_bus_count(), bus_count);
