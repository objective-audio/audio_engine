...
- (void)test_create_connection_failed {
    ...
    XCTAssertThrows(
        audio::connection::testable::create(null_node, source_bus, destination_node, destination_bus, format));
    XCTAssertThrows(audio::connection::testable::create(source_node, source_bus, null_node, destination_bus, format));
}
