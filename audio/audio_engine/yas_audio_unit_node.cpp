...
struct audio::engine::unit_node::impl : base::impl, manageable_unit_node::impl {
    ...
    void prepare(unit_node const &node, AudioComponentDescription const &acd) {
        ...
        _reset_observer = _node.subject().make_observer(audio::engine::node::method::will_reset, [weak_node](auto const &) {
            if (auto node = weak_node.lock()) {
                node.impl_ptr<audio::engine::unit_node::impl>()->will_reset();
            }
        });
        ...
        _connections_observer =
            _node.subject().make_observer(audio::engine::node::method::update_connections, [weak_node](auto const &) {
                if (auto node = weak_node.lock()) {
                    node.impl_ptr<audio::engine::unit_node::impl>()->update_unit_connections();
                }
            });
    ...
    void update_unit_connections() {
        _subject.notify(audio::engine::unit_node::method::will_update_connections, cast<audio::engine::unit_node>());
        ...
        _subject.notify(audio::engine::unit_node::method::did_update_connections, cast<audio::engine::unit_node>());
    }
    ...
    audio::engine::unit_node::subject_t _subject;
...
#pragma mark - audio::engine::unit_node

audio::engine::unit_node::unit_node(OSType const type, OSType const sub_type)
...
audio::engine::unit_node::unit_node(AudioComponentDescription const &acd)
    : unit_node({.acd = acd, .node_args = {.input_bus_count = 1, .output_bus_count = 1}}) {
}

audio::engine::unit_node::unit_node(args &&args) : base(std::make_shared<impl>(std::move(args.node_args))) {
    impl_ptr<impl>()->prepare(*this, args.acd);
}

audio::engine::unit_node::unit_node(std::nullptr_t) : base(nullptr) {
}

audio::engine::unit_node::~unit_node() = default;

void audio::engine::unit_node::set_prepare_audio_unit_handler(prepare_au_f handler) {
    impl_ptr<impl>()->set_prepare_audio_unit_handler(std::move(handler));
}

audio::unit audio::engine::unit_node::audio_unit() const {
    return impl_ptr<impl>()->au();
}

std::unordered_map<AudioUnitParameterID, audio::unit::parameter_map_t> const &audio::engine::unit_node::parameters() const {
    return impl_ptr<impl>()->parameters();
}

audio::unit::parameter_map_t const &audio::engine::unit_node::global_parameters() const {
    return impl_ptr<impl>()->global_parameters();
}

audio::unit::parameter_map_t const &audio::engine::unit_node::input_parameters() const {
    return impl_ptr<impl>()->input_parameters();
}

audio::unit::parameter_map_t const &audio::engine::unit_node::output_parameters() const {
    return impl_ptr<impl>()->output_parameters();
}

uint32_t audio::engine::unit_node::input_element_count() const {
    return impl_ptr<impl>()->input_element_count();
}

uint32_t audio::engine::unit_node::output_element_count() const {
    return impl_ptr<impl>()->output_element_count();
}

void audio::engine::unit_node::set_global_parameter_value(AudioUnitParameterID const parameter_id, float const value) {
    impl_ptr<impl>()->set_global_parameter_value(parameter_id, value);
}

float audio::engine::unit_node::global_parameter_value(AudioUnitParameterID const parameter_id) const {
    return impl_ptr<impl>()->global_parameter_value(parameter_id);
}

void audio::engine::unit_node::set_input_parameter_value(AudioUnitParameterID const parameter_id, float const value,
                                                 AudioUnitElement const element) {
    impl_ptr<impl>()->set_input_parameter_value(parameter_id, value, element);
}

float audio::engine::unit_node::input_parameter_value(AudioUnitParameterID const parameter_id,
                                              AudioUnitElement const element) const {
    return impl_ptr<impl>()->input_parameter_value(parameter_id, element);
}

void audio::engine::unit_node::set_output_parameter_value(AudioUnitParameterID const parameter_id, float const value,
                                                  AudioUnitElement const element) {
    impl_ptr<impl>()->set_output_parameter_value(parameter_id, value, element);
}

float audio::engine::unit_node::output_parameter_value(AudioUnitParameterID const parameter_id,
                                               AudioUnitElement const element) const {
    return impl_ptr<impl>()->output_parameter_value(parameter_id, element);
}

audio::engine::unit_node::subject_t &audio::engine::unit_node::subject() {
    return impl_ptr<impl>()->_subject;
}

audio::engine::node const &audio::engine::unit_node::node() const {
    return impl_ptr<impl>()->_node;
}

audio::engine::node &audio::engine::unit_node::node() {
    return impl_ptr<impl>()->_node;
}
