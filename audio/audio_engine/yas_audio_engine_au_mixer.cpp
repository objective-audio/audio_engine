...
float audio::engine::au_mixer::output_volume(uint32_t const bus_idx) const {
    return this->_au->output_parameter_value(kMultiChannelMixerParam_Volume, bus_idx);
}

void audio::engine::au_mixer::set_output_pan(float const pan, uint32_t const bus_idx) {
    this->_au->set_output_parameter_value(kMultiChannelMixerParam_Pan, pan, bus_idx);
}

float audio::engine::au_mixer::output_pan(uint32_t const bus_idx) const {
    return this->_au->output_parameter_value(kMultiChannelMixerParam_Pan, bus_idx);
}

void audio::engine::au_mixer::set_input_volume(float const volume, uint32_t const bus_idx) {
    this->_au->set_input_parameter_value(kMultiChannelMixerParam_Volume, volume, bus_idx);
}

float audio::engine::au_mixer::input_volume(uint32_t const bus_idx) const {
    return this->_au->input_parameter_value(kMultiChannelMixerParam_Volume, bus_idx);
}

void audio::engine::au_mixer::set_input_pan(float const pan, uint32_t const bus_idx) {
    this->_au->set_input_parameter_value(kMultiChannelMixerParam_Pan, pan, bus_idx);
}

float audio::engine::au_mixer::input_pan(uint32_t const bus_idx) const {
    return this->_au->input_parameter_value(kMultiChannelMixerParam_Pan, bus_idx);
}

void audio::engine::au_mixer::set_input_enabled(bool const enabled, uint32_t const bus_idx) {
    this->_au->set_input_parameter_value(kMultiChannelMixerParam_Enable, enabled ? 1.0f : 0.0f, bus_idx);
}
