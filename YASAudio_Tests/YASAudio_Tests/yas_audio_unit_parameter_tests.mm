...
- (void)test_create
{
    ...
    yas::audio::audio_unit::parameter parameter{info, parameterID, scope};
...
{
    yas::audio::audio_unit::parameter parameter{AudioUnitParameterInfo{}, 0, 0};

    bool called = false;

    auto will_observer = parameter.subject().make_observer(
        yas::audio::audio_unit::parameter::will_change_key,
        [self, &called](const auto &key, const yas::audio::audio_unit::parameter::change_info &change_info) {
            XCTAssertTrue(key == yas::audio::audio_unit::parameter::will_change_key);
...
- (void)test_subject_did_change
{
    ...
    auto will_observer = parameter.subject().make_observer(
        yas::audio::audio_unit::parameter::did_change_key,
        [self, &called](const auto &key, const yas::audio::audio_unit::parameter::change_info &change_info) {
            XCTAssertTrue(key == yas::audio::audio_unit::parameter::did_change_key);
            const AudioUnitElement element = change_info.element;
