...
- (void)testIORendering
{
    ...
    YASAudioFormat *outputFormat = [[YASAudioFormat alloc] initStandardFormatWithSampleRate:outputSampleRate channels:channels];
    YASAudioFormat *mixerFormat = [[YASAudioFormat alloc] initStandardFormatWithSampleRate:mixerSampleRate channels:channels];
    ...
    YASAudioUnit *mixerUnit = [audioGraph addAudioUnitWithType:kAudioUnitType_Mixer subType:kAudioUnitSubType_MultiChannelMixer prepareBlock:^(YASAudioUnit *audioUnit) {
    ...
    audioIOUnit.renderCallbackBlock = ^(YASAudioUnitRenderParameters *renderParameters) {
        ...
        for (UInt32 i = 0; i < outputFormat.bufferCount; i++) {
            ...
            XCTAssertEqual(ioData->mBuffers[i].mDataByteSize, outputFormat.sampleByteCount * outputFormat.stride * renderParameters->inNumberFrames);
        }
...
- (void)testInterrupting
{
    ...
    NSNotification *beganNotification = [[NSNotification alloc] initWithName:AVAudioSessionInterruptionNotification object:nil userInfo:beganUserInfo];
    ...
    NSNotification *endedNotification = [[NSNotification alloc] initWithName:AVAudioSessionInterruptionNotification object:nil userInfo:endedUserInfo];
